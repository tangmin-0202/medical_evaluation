...
> 2026-09-01 后续确认的 CP01 笔接触门控、接触前背景排除、接触后新增黑点跟踪及“Top 2 中取距离固定点最近者”规则，以 `2026-09-01-cp01-pen-gated-mark-tracking-design.md` 为最终定义；该文档覆盖本节中与其冲突的角落辅助点规则。
...
- CP11 状态严格区分：全阶段从未出现橡皮布为 `incomplete`；阶段中出现过但末尾约 3 秒消失为 `incorrect`，且该路径不要求伪造末尾提示或运行 SAM2；只有末尾仍有橡皮布才需要 3–5 个 `rubber_dam` 正点和一个 `nose_region` 框继续判断面积、鼻部遮挡与支架覆盖。
- CP01 暗点检测保留静态暗色连通域与跨帧稳定性方案，通过放宽最小面积和亮度上限覆盖极小、较浅的低饱和度黑灰笔点，同时用饱和度排除深绿色网格和文字，不改为时序差分。CP11 使用 SAM2 橡皮布掩膜与确定性绿色像素掩膜的交集计算面积和鼻部重叠；支架仍从 CP09 连续跟踪至 CP11，白色证据轮廓只显示末尾仍符合 CP09 支架外观的像素。
