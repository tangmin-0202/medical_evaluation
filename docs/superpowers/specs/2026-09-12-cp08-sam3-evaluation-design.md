...
## 2026-09-14 用户批准的规则变更（优先于下文旧版尖端规则）

本轮三个视频 Demo 改用器具外形代理识别，不再证明工作末端实际钝尖。
SAM3 自动分割候选后，需要可见长金属柄、细工作杆和弯曲工作端的匹配证据；
仅有一段杆柄不算匹配。清晰匹配帧即可，不要求全程追踪，也不因最末端钝尖不可辨而复核。
证据不足是外形被遮挡或候选掩膜不可靠，而不是末端圆度不可靠。
尖头文本提示只保留为分割对照，不进入正式状态或扣分判定。
报告使用“观察到符合约定外形的器具”，不得声称“已证明末端为钝头”。
实现特征改为 `instrument_shape_match`、`instrument_shape_reliable` 和可见匹配帧数；
旧 `tip_*`、`sharp_probe_used`、`unreliable_instrument_tip` 不用于本轮正式 Judge。
CP09 末尾双翼完整露出及双孔同源绿色规则不变。
截图仅用于语义确认，不作为运行时人工点、框、掩膜或匹配真值。
当前只能验证 Demo 外形代理，不能宣称区分其他器具的性能。
...
- 标注 JSON 中已有的器具点提示属于历史人工标注，本方案明确不读取、不使用这些点；器具位置、首次出现时刻和候选类别必须由 SAM3 文本提示在 CP08 阶段内自动搜索得到。当前仍允许使用人工给出的 CP08/CP09 阶段起止时间，因为整个项目尚处于“人工阶段时间 → 自动视觉评价”的既定阶段。
- 该边界与当前 CP09/CP11 的真实 SAM3 主路径一致：`TextPromptPolicy` 生成文本提示且忽略标注对象 prompt；CP10 由 OpenCV 自动建立目标牙和牙线证据。仓库保留的 `AnnotationPromptPolicy` 只是旧后端兼容路径，不作为本次 CP08 实现依据。
...
SAM3 的语义提示提供候选；末端宽度、圆度、尖锐度、渐缩程度和跨帧一致性共同形成确定性类别特征。当前截图只用于澄清“钝头工作端”的语义，不作为运行时输入、定位提示或自动判定真值。工程验收查看 SAM3 自动找到的候选帧及其相邻帧；尖端被手遮挡、运动模糊或掩膜断裂时，不强行分类。
